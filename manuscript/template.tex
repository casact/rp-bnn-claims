...
In this section, we briefly discuss Bayesian inference on neural networks, following the discourse in \cite{graves2011practical} and \cite{blundell2015weight}. We remark that in this subsection and the next, we discuss Bayesian inference and mixture density networks in general, and provide details on our specific choices of distributions later on in the paper.
...
\begin{equation}
    \mathcal{F}_i(\mathcal{D}_i, \theta) = \frac{1}{M}D_{KL}(q(w|\theta) \Vert P(w)) - \mathbb{E}_{q(w|\theta)}[\log P(\mathcal{D}_i|w)],
\end{equation}

which we approximate with

\begin{equation}
    \mathcal{F}_i(\mathcal{D}_i, \theta) \approx \sum_{j=1}^{n^{(m)}} \log q(w^{(j)}|\theta) - \log P(w^{(j)}) - \log P(\mathcal{D}_i|w^{(j)}),
\end{equation}

where the $w^{(j)}$ are sampled independently from $q(w|\theta)$ and $n^{(m)}$ denotes the number of samples in batch $m$. In other words, we sample from the weights distribution just once for each training sample.
